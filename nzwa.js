...
async function starts() {
	...
	nzwa.on('open', () => {
		success('2', '[BOT] Hideri BOT agora está online!')
	})
	await nzwa.connect({timeoutMs: 30*1000})
        ...
        lolcatjs.fromString('[DEV] Bem-vindod de volta proprietário!Espero que você esteja bem-')
		...
		try {
			...
			const isCmd = body.startsWith(prefix)
                        ...
                        mess = {
                                ...
                                levelon: '❬ ✔ ❭ *Nivel foi ativo*',
                                levelnoton: `❬ X ❭  *Nivel desativado*`,
                                levelnol: 'LEVEL 0 Kskst',
                                error: {
                                	    stick: '[❗] Falha, ocorreu um erro ao converter a imagem para o sticker ❌',
                                        Iv: '❌ Link inválido ❌',
                                },
                        	only: {
                        	        group: '[❗] Este comando só pode ser usado no grupo! ❌',
			...
			const levelRole = getLevelingLevel(sender, _level)
   	                var role = 'Aldeão'
   	                if (levelRole <= 3) {
   	                        role = 'Heroi Rank Cobre'
   	                } else if (levelRole <= 5) {
   	                        role = 'Heroi Rank Ferro '
   	                } else if (levelRole <= 7) {
   	                        role = 'Heroi Rank Prata'
   	                } else if (levelRole <= 8) {
   	                        role = 'Heroi Rank Ouro'
   	                } else if (levelRole <= 9) {
   	                        role = 'Heroi Rank Platina'
   	                } else if (levelRole <= 10) {
   	                        role = 'Heroi Rank Mythril'
   	                } else if (levelRole <= 11) {
   	                        role = 'Heroi Rank Orichalcum'
   	                } else if (levelRole <= 12) {
   	                        role = 'Heroi Rank Adamantite'
   	                } else if (levelRole <= 13) {
   	                        role = 'Arqui Demonio I'
   	                } else if (levelRole <= 14) {
   	                        role = 'Arqui Demonio II'
   	                } else if (levelRole <= 14) {
   	                        role = 'Arqui Demonio III'
                ...
                if (isGroup) {
					...
					reply(`Desculpe, o membro do grupo ainda não foi qualificado.Grupo membro mínimo é ${memberlimit}`)
                                        setTimeout( () => {
		        ...
		        if (isGroupAdmins) return reply('Porque vc e um Admin naum levara ban')
		        nzwa.updatePresence(from, Presence.composing)
		        if (messagesC.includes("#izinadmin")) return reply("#Izinadmin recebeu")
		        var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		        reply(`Opoha link de grupo detectado desculpe ${sender.split("@")[0]} sera expulso pelo link adios`)
		        setTimeout( () => {
 	   	...
 	   	if (isGroup && isBadWord) {
                        if (bad.includes(messagesC)) {
                                if (!isGroupAdmins) {
                                        return reply("Cuide do seu pinto!!😠.")
                                        .then(() => nzwa.groupRemove(from, sender))
                                        .then(() => {
                                                nzwa.sendMessage(from, `*「 ANTI BADWORD 」*\nVocê dikick porque ele disse rudemente!`, text ,{quoted: mek})
                                        }).catch(() => nzwa.sendMessage(from, `Felizmente vc não é um administrador, se o administrador tiver um chute cya!`, text , {quoted : mek}))
                                } else {
                                        return reply( "Por favor, cuide da discurso médio 😇")
                                }
                        }
                }
		...
		nzwa.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Demote bem sucedido @${mentioned[0].split('@')[0]} Ser um membro do grupo!`)
						nzwa.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Comando recebido, adicionando o departamento como administrador :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Comando recebido, adicionando o departamento como administrador : @${mentioned[0].split('@')[0]}`, mentioned, true)
						nzwa.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
				  case 'wame':
                                          ...
                                          console.log('Obtendo a imagem do perfil do whatsapp...')
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               nzwa.sendMessage(from, 'Aqui está o número do meu dono, salvar, mano, então vai voltar',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                nzwa.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
              ...
              await nzwa.sendMessage(from, options, text)
               break
                                case 'tiktokstalk':
					try {
						if (args.length < 1) return nzwa.sendMessage(from, 'Nome? ', text, {quoted: mek})
                                                if (!isRegister) return reply(mess.only.daftarB)
                                                if (isLimit(sender)) return reply(ind.limitend(pusname))
		...
		case 'infonomor':
               nzwa.updatePresence(from, Presence.composing) 
                 if (!isRegister) return reply(mess.only.daftarB)
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
                 if (args.length < 1) return reply(`Insira numeros\Eexemplo : ${prefix}infonomor 0812345678`)
                ...
                break
                   ...
                   nzwa.sendMessage(from, hasil, image, {quoted: mek, caption: `Aer *${body.slice(5)}*`})
                   await limitAdd(sender)
                   break
                   case 'covidcountry':
                   nzwa.updatePresence(from, Presence.composing) 
				...
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('Quem quer escrever?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					teks = body.slice(7)
					reply(mess.wait)
	...
	case 'kbbi':
            nzwa.updatePresence(from, Presence.composing) 
                if (args.length < 1) return reply(`Exemplo : ${prefix}kbbi ${pushname}`)
                if (!isRegister) return reply(mess.only.daftarB)
                if (isLimit(sender)) return reply(ind.limitend(pusname))
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
                ...
                break
				case 'blocklist':
					teks = 'Esta é a lista de número bloqueado :\n'
                   ...
                   case 'chatlist':
					nzwa.updatePresence(from, Presence.composing)  
					teks = 'Esta é a lista de número de bate-papo :\n'
				...
				case 'tts':
				   ...
				   if (args.length < 1) return nzwa.sendMessage(from, 'Onde é o código de idioma?', text, {quoted: mek})
                                   if (!isRegister) return reply(mess.only.daftarB)
                                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return nzwa.sendMessage(from, 'Onde está o texto', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('O texto é principalmente om.')
				...
				case 'adminlist':
					nzwa.updatePresence(from, Presence.composing) 
                                        if (!isRegister) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista admin de grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
                ...
                case 'pinterest':
					...
					nimek =  n[Math.floor(Math.random() * n.length)];
					pok = await getBuffer(nimek)
					nzwa.sendMessage(from, pok, image, { quoted: mek, caption: `Bem ${pushname} tinha muitas imagens mais escolhi essa espero que goste : *${tels}*`})
                                        await limitAdd(sender)
					break
				case 'setprefix':
					nzwa.updatePresence(from, Presence.composing) 
					...
					prefix = args[0]
					reply(`O prefix é alterado para ser : ${prefix}`)
					break
				case 'meme':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'block':
					...
					nzwa.sendMessage(from, `pedidos recebidos, bloqueando ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					nzwa.updatePresence(from, Presence.composing) 
				...
				case 'tagall':
					...
					for (let mem of groupMembers) {
						teks += `Tag @${mem.jid.split('@')[0]}\n`
						members_id.push(mem.jid)
					}
					mentions('Marcando\n'+teks+'Hideri Bot', members_id, true)
					break
                case 'tagall2':
				nzwa.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					nzwa.sendMessage(from, 'Marcando\n'+teks+'Hideri Bot', text, {quoted: mek})
					break
                case 'tagall3':
				nzwa.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					teks += `  Total : ${groupMembers.length}\n`
					for (let mem of groupMembers) {
						teks += `Tag https://wa.me/${mem.jid.split('@')[0]}\n`
						members_id.push(mem.jid)
					}
					nzwa.sendMessage(from, 'Marcando\n'+teks+'Hideri Bot', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				nzwa.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				nzwa.updatePresence(from, Presence.composing) 
					...
					teks += `  Total : ${groupMembers.length}\n`
					for (let mem of groupMembers) {
						teks += `Tag ${mem.jid.split('@')[0]}@s.whatsapp.net\n`
						members_id.push(mem.jid)
					}
					reply('Marcando\n'+teks+'Hideri Bot')
					break
				...
				case 'setppbot':
				nzwa.updatePresence(from, Presence.composing) 
				if (!isQuotedImage) return reply(`Envie fotos com legendas ${prefix}setbotpp ou a tag de imagem que foi enviada`)
					if (!isOwner) return reply(ind.ownerB())
					enmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await nzwa.downloadAndSaveMediaMessage(enmedia)
					await nzwa.updateProfilePicture(botNumber, media)
					reply('Obrigado o novo perfil -~- ')
					break
				case 'bc':
					nzwa.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(ind.ownerB())
				...
				case 'alay':
					...
					nzwa.groupLeave (from) 
                     ...
                     setTimeout( () => {
					nzwa.updatePresence(from, Presence.composing) 
					nzwa.sendMessage(from, 'Xauzinhuuu', text) // ur cods
					}, 0)
                     break

				case 'chord':
					if (args.length < 1) return reply('onde esta o título?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)					
               ...
               nzwa.updatePresence(from, Presence.composing) 
              options = {
          text: `O proprietário do grupo é: @${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Quem voce quer adiconar?')
					...
					try {
						...
						console.log('Error :', e)
						reply('Não conseguiu adicionar um alvo, talvez porque é privado')
					}
					break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`A ordem recebida, emitida : @${mentioned[0].split('@')[0]}`, mentioned, true)
						nzwa.groupRemove(from, mentioned)
					nzwa.sendMessage(mentioned, 'Okei Senor', text)
					}
					break
				case 'exe':
	              nzwa.updatePresence(from, Presence.composing) 
	              if (!isOwner) return reply(ind.ownerB())
	               ...
	               exec(cmd, (err, stdout) => {
		           if(err) return nzwa.sendMessage(from, "Comando Salah", text, { quoted: mek })
		           if (stdout) {
				...
				case 'ocr':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Apenas uma foto apenas')
					}
					break

                      case 'bugreport':
                     const bug = body.slice(5)
                      if (pesan.length > 300) return nzwa.sendMessage(from, 'Desculpe o texto é muito longo, um máximo de 300 textos', msgType.text, {quoted: mek})
                    ...
                    reply('Caso sejam relatorios falsos/Naum sera respondido.')
                    break
               case 'apakah':
               nzwa.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo fechado por admin @${nomor.split("@s.whatsapp.net")[0]}\nagora *Apenas Admin* Quem pode enviar mensagens`,
					contextInfo: { mentionedJid: [nomor] }
					}
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Falhou, ao converter vídeo para mp3')
						buffer = fs.readFileSync(ran)
	          ...
	          if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Músicas encontradas!!!*\nTítulo : ${anu.result.title}\nFonte : ${anu.result.source}\nTamanho : ${anu.result.size}\n\n*Espere um minuto em breve, por favor, naum spam*`
                ...
                break
                   ...
                   break
					...
					setTimeout( () => {
					nzwa.sendMessage(from, '*❑ Responda :* '+anu.result.jawaban+'\n'+anu.result.desk, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					...
					if (args.length < 1) return reply('Tipo 1 para ativar')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Os recursos estão ativos')
						welkom.push(from)
						...
						reply('Tipo 1 para ativar, 0 para desativar o recurso')
					}
                                        break
                                case 'fakta':
					fakta = body.slice(1)
					...
					if (args.length < 1) return reply(mess.blank)
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
					if (tels.length > 15) return reply('O texto é extenso, no máximo 20 caracteres')
					reply(mess.wait)
				...
				case 'firetext':
					if (args.length < 1) return reply(mess.blank)
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
					if (tels.ength > 10) return reply('O texto é extenso, no máximo 9 caracteres')
					reply(mess.wait)
					...
					nzwa.sendMessage(from, 'Pergunta : *'+ganteng+'*\n\nResposta : '+ teng+'%', text, { quoted: mek })
					break
					case 'cantikcek':
					if (isRegister) return reply(mess.only.daftarB)
					cantik = body.slice(1)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`qual texto Exemplo ${prefix}logowolf ${pushname}|Hiderir`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
					...
					if (args.length < 1) return reply('Tipo 1 para ativar')
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('Os recursos estão ativos')
						nsfw.push(from)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('Sucesso NSFW ativo neste grupo')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('Sucesso NSFW desativado neste grupo')
					} else {
						reply('Tipo 1 para ativar, 0 para desativar o recurso')
					}
					break	
            ...
            case 'leveling':
                ...
                if (args[0] === '1') {
                    if (isLevelingOn) return reply('*O recurso de nível estava ativo antes*')
                    ...
                    reply(` *Digite comando 1 para ativar, 0 para desativar* \n *Exemplo: ${prefix}leveling 1*`)
                }
            break
                                case 'infogempa':
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                ...
                                case 'shadow':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}shadow ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}burnpaper ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'coffee':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}coffee ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}loverpaper ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'woodblock':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}woodblock ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'qowheart':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}qowheart ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}mutgrass ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}undergocean ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'woodenboards':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}woodenboards ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'wolfmetal':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}wolfmetal ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}metalictglow ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case '8bit':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`O texto???\nExemplo: ${prefix}8bit ${pushname}|Hideir`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
				...
				case 'ttp':
					...
					break
				case 'simi':
					if (args.length < 1) return reply('Texto é onde um?')
					teks = body.slice(5)
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(ind.ownerB())
					if (args.length < 1) return reply('Tags de destino querem clonar')
					...
					try {
						...
						mentions(`Brigaduuh ${pushname} pela nova foto de perfil`, [jid], true)
					} catch (e) {
						reply('Gagal om')
					}
					break
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`Texto...\nExemplo: ${prefix}pubglogo ${pushname}|Rola`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                ...
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply(`Texto...\nExemplo: ${prefix}harrypotter ${pushname}`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                ...
                                case 'bal':
                                        ...
                                        break
                                case 'buylimit':
                                        if (args.length < 1) return reply(`Oiin ${pushname} queres quanto??! \n\nComo verificar dinheiro: ${prefix}bal`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        payout = body.slice(10)
                                        const koinPerlimit = 1000
                                        const total = koinPerlimit * payout
                                        if ( checkATMuser(sender) <= total) return reply(`Desculpe seu dinheiro é insuficiente. Por favor colete e compre mais tarde`)
                                        if ( checkATMuser(sender) >= total ) {
                                                confirmATM(sender, total)
                                                bayarLimit(sender, payout)
                                                await reply(`*「 Resultados de pagamento 」*\n\n*remetente* : Admin\nreceptora* : ${pushname}\n*compra nominal* : ${payout} \n *Limite Preço* : ${koinPerlimit}/Limite *Obs Restante*: ${checkATMuser(sender)}\n\nProcesso bem-sucedido com números de pagamento \n${createSerial(15)}`)
                                        }
                                        break
                                case 'limit':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                ...
                                case 'event':
                                        ...
                                        if (Number(args[0]) === 1) {
                                                ...
                                                reply('*「 Sucesso. 」 Ative o evento neste grupo*')
                                        } else if (Number(args[0]) === 0) {
                                                event.splice(from, 1)
                                                fs.writeFileSync('./database/json/event.json', JSON.stringify(event))
                                                reply('*「 Sucesso. 」 Desativar o evento neste grupo*')
                                        } else {
                                                reply(ind.satukos())
                                        }
                                        break
                                case 'register':
                                        ...
                                        if (namaUser.length >= 30) return reply(`Por que seu nome é tao grande??`)
                                        if (umurUser > 40) return reply(`Idade Maxima 40`)
                                        if (umurUser < 12) return reply(`Idade Minima 12`)
                                        veri = sender
                                        if (isGroup) {
				        ...
				        break
                                case 'mining':
                                        ...
                                        if (isOwner) {
                                                ...
                                                reply(`Porque você é nosso proprietário do Equipe Bot Enviando ${one}Xp para você`)
                                        } else {
                                                const mining = Math.ceil(Math.random() * 10000)
                                                addLevelingXp(sender, mining)
                                                await reply(`*Parabéns* ${pushname} Você começa *${mining}Xp*`)
                                        }
                                        await limitAdd(sender)
                                        break
                                 /*case 'setname':
					...
					break*/
                                case 'translate':
                                        ...
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (args.length < 1) return reply(`onde é o código de idioma?\nExemplo: ${prefix}translate en|Oin, Hideri`)
                                        if (args.length < 2) return reply(`Seu texto onde é ta?\nExemplo: ${prefix}translate en|Oin, Hideri`)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                ...
                                case 'tafsirmimpi':
                                        aruga = body.slice(12)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (args.length < 1) return reply(`Sonhe o que?\nExemplo.: ${prefix}tafsirmimpi belanja`)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                ...
                                case 'tagme':
                                        ...
                                        var nom = mek.participant
                                        const tagme = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} Aer!`,
                                                contextInfo: { mentionedJid: [nom] }
                                        }
                                ...
                                case 'ip': // masih testing
                                        ipnya = body.slice(3)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (args.length < 1) return reply(`Ip onde? \nExemplo: ${prefix}ip 8.8.8.8`)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                ...
                                case 'happymod':
                                        toby = body.slice(10)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (args.length < 1) return reply(`Nome do jogo? \nExemplo : ${prefix}happymod pubg`)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                ...
                                case 'say':
                                        teks = body.slice(5)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (args.length < 1) return reply('seu texto?')
                                        saying = teks
					...
					if (args.length < 1) return reply('Tipo 1 para ativar')
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('Anti Link Group está ativo')
						antilink.push(from)
						fs.writeFileSync('./database/json/antilink.json', JSON.stringify(antilink))
						reply('Sucesso permite grupo anti-vinculação neste grupo ✔️')
						nzwa.sendMessage(from,`Atenção a todos os grupos anti-link ativos quando você envia um grupo de link você estará em chute do grupo`, text)
					} else if (Number(args[0]) === 0) {
						if (!isantilink) return reply('O modo de grupo de vinculação foi desativado')
						var ini = anti.indexOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./database/json/antilink.json', JSON.stringify(antilink))
						reply('Bem sucedido desabilitando grupos anti-link neste grupo ✔️')
					} else {
						reply('1 para ativar, 0 para desativar')
					}
					break
				...
				case 'lb':
				        ...
				        _level.sort((a, b) => (a.xp < b.xp) ? 1 : -1)
			         	uang.sort((a, b) => (a.uang < b.uang) ? 1 : -1)
                                        ...
                                        let leaderboarduang = '「 *classificação de líderes* 」\n\n'
                                        nom = 0
                                        try {
                                                for (let i = 0; i < 10; i++) {
                                                        nom++
                                                ...
                                                console.error(err)
                                                await reply(`minimal ${len} Usuário para poder acessar o banco de dados`)
                                        }
				        break
                                case 'antibadwordgroup':
                                        if (!isGroup) return reply(mess.only.group)
                                        if (!isGroupAdmins) return reply(ind.admin())
                                        if (args.length < 1) return reply('Tipo Ativar para ativar')
                                        if (args[0] === 'enable') {
                                                if (isBadWord) return reply('*O recurso Badword está ativo antes*')
                 	                        badword.push(from)
                 	                        fs.writeFileSync('./database/json/badword.json', JSON.stringify(badword))
                  	                        reply(`*[ Sucesso. ] Ativar Badword neste grupo!*`)
                                        } else if (args[0] === 'disable') {
                  	                        badword.splice(from, 1)
                 	                        fs.writeFileSync('./database/json/badword.json', JSON.stringify(badword))
                 	                        reply(`Badword é desativada`)
             	                        } else {
                 	                        reply(ind.satukos())
                	                }
                                        break
                                case 'addbadword':
                                        if (!isOwner) return reply(ind.ownerb())
                                        if (!isGroupAdmins) return reply(ind.admin())
                                        if (args.length < 1) return reply( `Enviar pedidos ${prefix}addbadword [palavras duras]. exemplo ${prefix}addbadword bego`)
                                        const bw = body.slice(12)
                                        bad.push(bw)
                                        fs.writeFileSync('./database/json/bad.json', JSON.stringify(bad))
                                        reply('Sucesso adicionado palavra ruim!')
                                        break
                                case 'delbadword':
                                        if (!isOwner) return reply(ind.ownerb())
                                        if (!isGroupAdmins) return reply(ind.admin())
                                        ...
                                        bad.splice(dbw)
                                        fs.writeFileSync('./database/json/bad.json', JSON.stringify(bad))
                                        reply('O sucesso remove a palavra ruim!')
                                        break 
                                case 'listbadword':
                                        let lbw = `Esta é uma lista de má palavra\nTotal : ${bad.length}\n`
                                        for (let i of bad) {
                                                lbw += `➸ ${i.replace(bad)}\n`
                                        }
                                        await reply(lbw)
			...
			case 'ssweb':
                if (!isRegister) return reply(mess.only.daftarB)
                    if (isBanned) return reply(mess.only.benned)    
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply('Cade a Url beber?')
					teks = body.slice(7)
					reply(ind.wait())
				...
				if (!isRegister) return reply(mess.only.daftarB)
					...
					if (args.length < 1) return reply('Seu texto onde? >.<')
					teks = body.slice(7)
					if (teks.length > 9) return reply('O texto é bonito, No máximo 9 caracteres')
					reply(mess.wait)
				    ...
				    if (!isRegister) return reply(mess.only.daftarB)
                     const cfrr = body.slice(9)
                      if (cfrr.length > 300) return nzwa.sendMessage(from, 'Desculpe texto é muito longo, um máximo de 300 textos', msgType.text, {quoted: mek})
                        var nomor = mek.participant
                    ...
                    reply('Pedido que você chegou no Bot do proprietário, solicitações falsas/main2 não serão respondidas.')
                    break
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[AVISSO]','red'), 'Comando não registrado de', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
